...
const listaMisVehiculos = (ci) =>{
    return new Promise((res, error) =>{
        mysqlConnection.query('SELECT vehiculo.placa, marca, serie, modelo, combustion, vehiculo.imagen, dispositivo FROM usuario_vehiculo, vehiculo, usuario WHERE usuario_vehiculo.placa = vehiculo.placa and usuario_vehiculo.ci= ?', [ci], (err, rows) => {
            if(!err){
                res(rows)
            } else {
                console.log(err)
